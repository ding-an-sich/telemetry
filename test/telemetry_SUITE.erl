...
all() ->
    [
        ...
        remove_all_handler_on_failure,
        list_handler_on_many,
        detach_from_all,
        old_execute,
        default_metadata,
        off_execute,
        invoke_successful_span_handlers,
        invoke_exception_span_handlers,
...
bad_event_names(Config) ->
    HandlerId = ?config(id, Config),
    ?assertError(
        badarg, telemetry:attach(HandlerId, ["some", event], fun ?MODULE:echo_event/4, [])
    ),
...
invoke_successful_span_handlers(Config) ->
    ...
    telemetry:attach_many(
        HandlerId, [StartEvent, StopEvent], fun ?MODULE:echo_event/4, HandlerConfig
    ),
    ...
    after 1000 -> ct:fail(timeout_receive_echo)
    end,
...
logs_on_local_function(Config) ->
    ...
    logger:add_primary_filter(logs_on_local_function, {fun ?MODULE:send_logs/2, self()}),
    ...
    try
        ...
        after 1000 -> ct:fail(timeout_receive_log)
        end,
